...
#### **1 ) Catégorie de dysfonctionnementet temps de cohabitation en fonction du degrés de gêne**
...
```{r}
subplot(
  p_interactif, p1_interactif, p2_interactif, p3_interactif, p4_interactif,
  p5_interactif, p6_interactif, p7_interactif, p8_interactif, p9_interactif,
  nrows = 5, 
  shareX = TRUE,   # Partager l'axe X entre les sous-graphes
  shareY = TRUE,   # Partager l'axe Y entre les sous-graphes
  titleX = TRUE,
  titleY = TRUE,
  widths = c(0.45, 0.45)  # Largeurs des colonnes ; doit être égal au nombre de colonnes
) %>%
  layout(
    margin = list(l = 50, r = 50, t = 100, b = 50),  # Ajuster les marges autour des graphiques
    xaxis = list(domain = c(0, 1)),  # Couvrir l'ensemble de la largeur disponible
    xaxis2 = list(domain = c(0, 1)),  # Ajuster pour chaque sous-graphe (si nécessaire)
    yaxis = list(domain = c(0, 1))    # Couvrir l'ensemble de la hauteur disponible
  )
```
