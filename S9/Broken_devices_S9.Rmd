...
df_T <- data.frame(Etat=labels,Valeurs = matrice[2,])
...
combinaisons_etats <- function(num){
  ...
  for (i in 1:length(variable)){
    ligne=variable[i]
    ligne_char <- str_extract_all(variable[i], ".{2}")[[1]]
    ligne_num <- as.numeric(ligne_char)
    if (num %in% ligne_num){
      vec = c(vec,ligne)
      res <- table(vec)
      df = data.frame(Combinaisons = names(res), Nb = as.vector(res))
...
df_T <- data.frame(Etat=labels,Valeurs = matrice[2,],Pourcent=round(matrice[2,]*100/length(etat1),1))
...
df_T <- data.frame(Etat=labels,Valeurs = matrice[2,],Pourcent=round(matrice[2,]*100/length(etat2),1))
...
df_T <- data.frame(Raisons=factor(label_RAISON_UTIL_PB,levels=label_RAISON_UTIL_PB),Valeurs = mat_raison_cat[,1], Légende = factor(label_RAISON_UTIL_PB,levels=label_RAISON_UTIL_PB))
...
df_T <- data.frame(Raisons=factor(label_RAISON_UTIL_PB,levels=label_RAISON_UTIL_PB),Valeurs = mat_raison_cat[,2], Légende = factor(label_RAISON_UTIL_PB,levels=label_RAISON_UTIL_PB))
...
df_T <- data.frame(Raisons=factor(label_RAISON_UTIL_PB,levels=label_RAISON_UTIL_PB),Valeurs = mat_raison_cat[,3], Légende = factor(label_RAISON_UTIL_PB,levels=label_RAISON_UTIL_PB))
...
df_T <- data.frame(Raisons=factor(label_RAISON_UTIL_PB,levels=label_RAISON_UTIL_PB),Valeurs = mat_raison_cat[,4], Légende = factor(label_RAISON_UTIL_PB,levels=label_RAISON_UTIL_PB))
...
p <- ggplot(df_combined, aes(x = Categorie, y = valeurs, fill = GÊNE)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        axis.title.x = element_text( face = "bold"),
        axis.title.y = element_text(face = "bold",colour="red3"),legend.position="none") 
...
p1 <- ggplot(df_combined, aes(x = Categorie, y = valeurs, fill = GÊNE)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        axis.title.x = element_text( face = "bold"),
        axis.title.y = element_text(face = "bold"),legend.position="none") + scale_y_continuous(labels = function(x) abs(x), breaks = seq(-2000, 5000, by = 1000))
...
p9 <- ggplot(df_combined, aes(x = Categorie, y = valeurs, fill = GÊNE)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        axis.title.x = element_text( face = "bold"),
        axis.title.y = element_text(face = "bold"),legend.position="none") + scale_y_continuous(labels = function(x) abs(x), breaks = seq(-60, 200, by = 60))
