...
p <- ggplot(df_T, aes(x = Catégorie, y = Valeurs, fill = Légende)) +
  ...
  scale_fill_manual(values =c("#FF0000", "#00FF00", "#0000FF", "#FFFF00", "#00FFFF", "#FF00FF", "#FFA500", "#800080", "#A52A2A", "#FFC0CB", "#808080", "#000000", "#FFFFFF","#00868B")) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        axis.title.x = element_text( face = "bold",colour="#00868B"), 
        axis.title.y = element_text(face = "bold",colour="#00868B"),  
...
p <- ggplot(df_T, aes(x = Catégorie, y = Valeurs, fill = Légende)) +
  geom_bar(stat = "identity", position = "stack") + # mettre position = "dodge" si on veut des barres côtes à côtes
  labs(title = "Degrés de gêne ressenti ",
...
Exemple de diverging bar charts :


```{r echo=FALSE}
# Exemple de données similaires à votre graphique
data1 <- data.frame(
  Category = rep(c("All Survey Responses", "Private consultant/self-employed", "Academic (nonstudent)", "Business and industry", "Federal, state, and local government", "Other (including retired, students, not employed, etc.)", "Asian", "White", "Other", "Black or African American", "Master's and Above", "Associate's and Bachelor's", "Male", "Female", "Important", "Not Important"), each = 2),
  SubCategory = rep(c("Negative", "Positive"), 16),
  Value = c(-10, 90, -5, 95, -15, 85, -10, 90, -8, 92, -7, 93, -20, 80, -12, 88, -6, 94, -3, 97, -4, 96, -11, 89, -18, 82, -2, 98, -9, 91, -1, 99)
)

# Création du graphique à barres divergentes
ggplot(data1, aes(x = Category, y = Value, fill = SubCategory)) +
  geom_bar(stat = "identity", position = "stack") +
  coord_flip() + 
  scale_fill_manual(values = c("Negative" = "red", "Positive" = "blue")) +
  labs(title = "Réponses au sondage par catégorie",
       x = "Catégorie",
       y = "Fréquence") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
Degrés de gêne et temps de cohabitation 
...
total = sum(matrice_REP[2,1:2])

Categorie <- factor(label_COHABITATION, levels = label_COHABITATION)

df_T1 <- data.frame(Categorie = Categorie, valeurs = (-1)*round(matrice[2,]*100/total,2), GÊNE = "Trés gênant")
...
df_T1 <- data.frame(Categorie = Categorie, valeurs = (matrice[2,]), GÊNE = "Trés gênant")
df_T2 <- data.frame(Categorie = Categorie, valeurs = matrice[3,], GÊNE = "Plutôt gênant")
df_T3 <- data.frame(Categorie = Categorie, valeurs = matrice[4,], GÊNE = "Plutôt pas gênant")
