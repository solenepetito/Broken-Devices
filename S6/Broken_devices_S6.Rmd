...
p <- ggplot(df_T, aes(x = Catégorie, y = Valeurs, fill = Légende)) +
  geom_bar(stat = "identity", position = "dodge") + # mettre position = "dodge" si on veut des barres côtes à côtes
...
for(i in 1:nrow(data)){
  for (j in 81:105){
    ...
    if (!is.na(val) && !is.null(val)){
      ...
      result <- which(matrice_cat == num_pb, arr.ind = TRUE)
      cat <- result[1]
      matrice[val,cat]=matrice[val,cat]+round(data[i,"poids"])    }
  }
}
...
print(matrice)
print(n) # 187 : nb de personnes rencontrant un pb
print(sum(matrice)) # 286 : nb d'usages rescencés pour ces 187 personnes
