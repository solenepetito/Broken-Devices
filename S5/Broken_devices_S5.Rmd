...
Précédemment il a été calculé le temps de cohabitation de ceux qui n'avait pas tenté de résoudre le(s) dysfonctionnement(s). Le graphe qui suit permet de montrer pour chaque temps de cohabitation, le degré de gêne qui a été ressentis pour chacun d'eux :
...
df_T1 <- data.frame(Categorie = Categorie, valeurs = mat[2,]*100/nb_non_resolus, Gêne = "Trés gênant")
df_T2 <- data.frame(Categorie = Categorie, valeurs = mat[3,]*100/nb_non_resolus, Gêne = "Plutôt gênant")
df_T3 <- data.frame(Categorie = Categorie, valeurs = mat[4,]*100/nb_non_resolus, Gêne = "Plutôt pas gênant")
df_T4 <- data.frame(Categorie = Categorie, valeurs = mat[5,]*100/nb_non_resolus, Gêne = "Pas gênant du tout")
...
subplot(
  ...
  layout(
    ...
    annotations = list(
      ...
      list(
        x = 0.8, y = 1.05, text = "Sur la même échelle", 
        showarrow = FALSE, xref = "paper", yref = "paper", xanchor = 'center', yanchor = 'bottom', font = list(size = 14, color = "#1f77b4", family = "Arial", weight = "bold")
      )
    ),
    margin = list(t = 100)  # Ajout de marge pour les titres
  )

```

Le premier graphe présente les degrés de gêne pour chaque temps de cohabitation pour 316 dysfonctionnements, et le deuxième graphe met tous ces temps de cohabitation à la même échelle et représente la part de chaque degré de gêne.

### **II ) Graphes qui rassemble les temps cohabitations pour tous les types de résolutions**

Le graphe suivant combine deux graphes indiquant le temps de cohabitation pour ceux qui ont résolu, ceux qui ont tenté de résoudre et qui n'ont pas réussi et ceux qui n'ont pas tenté de résoudre :
...
subplot(
  ...
  layout(
    title = list(
      text = "Répartion des temps de cohabitation",
      font = list(size = 25, color = "black", family = "Arial", face = "bold")
    ),
    annotations = list(
      list(
        x = 0.2, y = 1.05, text = "Sur 832 dysfonctionnements", 
        showarrow = FALSE, xref = "paper", yref = "paper", xanchor = 'center', yanchor = 'bottom', font = list(size = 14, color = "#1f77b4", family = "Arial", weight = "bold")
      ),
      list(
        x = 0.8, y = 1.05, text = "Sur la même échelle", 
...
```
Le premier graphe présente les types de résolution pour chaque temps de cohabitation, et le deuxième graphe met tous ces temps de cohabitation à la même échelle et représente la part de chaque type de résolution.

### **III ) Niveau de gêne de ceux qui ont réparé vs ceux qui n'ont pas réparé**

Le graphe suivant combine deux graphes indiquant le degré de gêne ressenti pour ceux qui ont résolu, ceux qui ont tenté de résoudre et qui n'ont pas réussi, et ceux qui n'ont pas tenté de résoudre :
...
Le premier graphe présente les =degrés de gêne pour chaque temps de résolution, et le deuxième graphe met tous ces types de résolution à la même échelle et représente la part de chaque degré de gêne.

### **IV ) Quels usages du smarthpone sont impactés par les dysfonctionnements ? **

Ici il n'a été concidéré que les personnes qui avait rencontrés un dysfonctionnement (187 personnes), et qui avaient par la suite listé les impacts que ce dysfonctionnement avait sur leur smartphone, ce qui a donnée le graphe suivant : 
```{r include=FALSE}
...
La lecture des informations n'étant pas trés lisible, les représentation qui suivent montrent pour 5 dysfonctionnements, les réparttion des usages impactés :  
...
### **V ) Graphes représentant les temps de cohabitation en fonction de la catégorie de dysfonctionnement et du degrés de gêne **
...
Le graphe suivant présente pour chaque temps de cohabitation la part de chaque degré de gêne, et dans ces parts, la part de chaque catégorie de dysfonctionnements : 
...
Le graphe suivant présente pour chaque temps de cohabitation la part de chaque catégorie de dysfonctionnments, et dans ces parts, la part de chaque degrés de gêne : 
