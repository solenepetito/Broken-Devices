...
### **I ) Pouecentage de personnes ne renconcontrant qu'un dysfonctionnement**
...
### Intention de remplacement en fonction de la durée d'usage du téléphone 
...
var = data[ data$ETATSMARTPHONE == 2, c("DATEOBTENTIONSMART_1", "DATEOBTENTIONSMART_2", "INTENTION_REMPLACEMENT", "ETATSMARTPHONE","DUREE_USAGE_ANCIEN_TEL")]
...
for (u in 1:nrow(var)) {
  ...
  ancienne_dur = var[u,"DUREE_USAGE_ANCIEN_TEL"]
  
  if (diff < 168 && !is.na(ancienne_duree)) {
    usage = c(usage, liste[1])
  } else if (diff >= 168 && diff < 365 && !is.na(ancienne_duree) && !is.null(ancienne_dur)) {
    usage = c(usage, liste[2])
    ancienne_duree = c(ancienne_duree,ancienne_dur)
  } else if (diff >= 365 && diff < 730 && !is.na(ancienne_duree)&& !is.null(ancienne_dur)) {
    usage = c(usage, liste[3])
    ancienne_duree = c(ancienne_duree,ancienne_dur)
  } else if (diff >= 730 && diff < 1095 && !is.na(ancienne_duree)&& !is.null(ancienne_dur)) {
    usage = c(usage, liste[4])
    ancienne_duree = c(ancienne_duree,ancienne_dur)
  } else if (diff >= 1095 && diff < 1825 && !is.na(ancienne_duree)&& !is.null(ancienne_dur)) {
    usage = c(usage, liste[5])
    ancienne_duree = c(ancienne_duree,ancienne_dur)
  } else if (diff >= 1825 && !is.na(ancienne_duree)&& !is.null(ancienne_dur)) {
...
# Filtrage et préparation des données
var <- data[data$ETATSMARTPHONE == 2, c("DATEOBTENTIONSMART_1", "DATEOBTENTIONSMART_2", "DUREE_USAGE_ANCIEN_TEL")]
