---
title: 'Broken devices : Module fin '
...
**OBJECTIF :** *Synthétiser les grahes du module fin*
...
df <- data.frame(RECONTACT =names(don), Nb_personnes = round(as.numeric(don),1), Pourcent= round(as.numeric(don)*100/sum(as.numeric(don)),1))

# df <- df %>% arrange( desc(Nb_personne))

df$RECONTACT=
  factor(df$RECONTACT,labels=c("Oui",
"Non"))
...
**Filtre** : Si RECONTACT = 1

Merci de nous partager votre e-mail :

/_________________/ @ /____________________/ . /____/

**REMARQUE :** Pour les e-mails, se référer au tableau excel des données brutes et aller à la dernière colonne
