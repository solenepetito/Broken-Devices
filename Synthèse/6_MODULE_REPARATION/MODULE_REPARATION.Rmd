---
title: 'Broken devices : Module réparation '
...
**OBJECTIF :** *Synthétiser les grahes du module réparation*
...
**Filtre du module :** si IDENTIFICATION_PROBLEME = 1 à 26 (au moins un
problème identifié en SAUF autre exclusif OU aucun)

## **I ) REPARATION**

Avez-vous tenté de résoudre ou de faire résoudre le(s)
dysfonctionnement(s) rencontré(s) ?

1.  Oui et le problème s'est résolu

2.  Oui mais le problème ne s'est pas résolu

3.  Non, le problème s'est résolu tout seul au bout d'un moment

4.  Non, peut être plus tard

5.  Non, je ne pense pas tenter de résoudre le problème
...
mat = matrix(0,nrow=25,ncol=5)

deb <- which(names(data) == "REPARATION_A1")
fin <- which(names(data) == "REPARATION_A25")
...
df <- data.frame(Problemes = label_RIDENTIFICATION_PROBLEME[1:25],`Oui et le problème s'est résolu` = mat[,1],`Oui mais le problème ne s'est pas résolu`
=mat[,2],`Non, le problème s'est résolu tout seul au bout d'un moment`=mat[,3],`Non, peut être plus tard`=mat[,4],`Non, je ne pense pas tenter de résoudre le problème`=mat[,5])


total_row <- data.frame(
  Problemes = "Total",
  `Oui.et.le.problème.s.est.résolu` = sum(df$Oui.et.le.problème.s.est.résolu),
  `Oui.mais.le.problème.ne.s.est.pas.résolu` = sum(df$Oui.mais.le.problème.ne.s.est.pas.résolu),
  `Non..le.problème.s.est.résolu.tout.seul.au.bout.d.un.moment` =sum(df$Non..le.problème.s.est.résolu.tout.seul.au.bout.d.un.moment),
  `Non..peut.être.plus.tard` =sum(df$Non..peut.être.plus.tard),
  `Non..je.ne.pense.pas.tenter.de.résoudre.le.problème` =sum(df$Non..je.ne.pense.pas.tenter.de.résoudre.le.problème)
)
...
## **II ) COHABITATION_DYSFONCTIONNEMENT**

**Filtre :** si REPARATION_1 à REPARATION_26 = 1 à 2 (Oui...)

Combien de temps s'est écoulé approximativement entre le moment où vous
avez décelé le(s) dysfonctionnement(s) et le moment où vous avez tenté
de le(s) résoudre ou de le(s) faire résoudre ?
...
mat = matrix(0,nrow=25,ncol=6)

deb <- which(names(data) == "COHABITATION_DYSFONCTION_A1")
fin <- which(names(data) == "COHABITATION_DYSFONCTION_A25")
...
df <- data.frame(Problemes = label_RIDENTIFICATION_PROBLEME[1:25],`Dès que j'ai décelé le dysfonctionnement` = mat[,1],`Moins d'un mois`
=mat[,2],`Entre 1 et 6 mois`=mat[,3],`Entre 6 mois et 2 ans`=mat[,4],`Plus de deux ans`=mat[,5],`Je ne sais pas`=mat[,6])


total_row <- data.frame(
  Problemes = "Total",
  `Dès.que.j.ai.décelé.le.dysfonctionnement` = sum(df$Dès.que.j.ai.décelé.le.dysfonctionnement),
  `Moins.d.un.mois` = sum(df$Moins.d.un.mois),
  `Entre.1.et.6.mois` =sum(df$Entre.1.et.6.mois),
  `Entre.6.mois.et.2.ans` =sum(df$Entre.6.mois.et.2.ans),
  `Plus.de.deux.ans` =sum(df$Plus.de.deux.ans),
...
**Filtre :** si REPARATION_1 à REPARATION_26 = 4 ou 5 (n'a pas réparé)

Vous nous avez déclaré ne pas avoir tenté de résoudre les
...
deb <- which(names(data) == "INT_REPARATION_A1")
fin <- which(names(data) == "INT_REPARATION_A25")
...
df <- data.frame(Problemes = label_RIDENTIFICATION_PROBLEME[1:25],`Oui, par moi-même` = mat[,1],`Oui, par le SAV ou un réparateur`
=mat[,2],`Oui, par une connaissance`=mat[,3],`Pas pour l'instant`=mat[,4],`Non`=mat[,5])


total_row <- data.frame(
  Problemes = "Total",
  `Oui..par.moi.même` = sum(df$Oui..par.moi.même),
  `Oui..par.le.SAV.ou.un.réparateur` = sum(df$Oui..par.le.SAV.ou.un.réparateur),
  `Oui..par.une.connaissance` =sum(df$Oui..par.une.connaissance),
  `Pas.pour.l.instant` =sum(df$Pas.pour.l.instant),
  `Non` =sum(df$Non)
)
