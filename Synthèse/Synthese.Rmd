...
```{r}
df = data.frame(Cohabitation = label_COHABITATION[2:6],Nb_dys=vec2)
df <- df %>% 
  arrange(desc(Nb_dys))
...
```{r}
df = data.frame(Recence_dys= label_RECENCE[vec],Nb_dys = matrice[1,])
...
```{r}
res = table(cohab_remplacement)
vec1 = as.numeric(names(res))
vec2 = as.numeric(res)

df = data.frame(Temps_ecoule = label , Nb_dys = vec2)
df <- df %>% 
  arrange(desc(Nb_dys))
