...
```{r}
COHAB_finale = COHAB_fin + 1
COHAB_finale = factor(COHAB_finale,levels =1:length(label_COHABITATION) ,labels=label_COHABITATION)

head(COHAB_finale)

resultat = table(COHAB_finale)
resultat

df <- data.frame(Cohabitation = label_COHABITATION[2:5], Valeurs = as.numeric(resultat[2:5] ))
df

```

```{r}
df <- data.frame(Cohabitation = factor(label_COHABITATION[2:5],levels=label_COHABITATION),valeurs=as.numeric(resultat[2:5]),
  start = c(0, 1, 6, 24),  # Début de l'intervalle en mois
  end = c(1, 6, 24, 48)    # Fin de l'intervalle en mois (Inf pour plus de 2 ans)
)

ggplot(df) +
  geom_rect(aes(xmin = start, xmax = end, ymin = 0, ymax = valeurs, fill = Cohabitation)) +
  scale_x_continuous(breaks = seq(0, 48, by = 6), limits = c(0, 48)) +  # Ajuster les limites et les breaks de l'axe des x
  scale_y_continuous(expand = c(0, 0)) +  # Ajuster les limites et les breaks de l'axe des y
  ...
  theme(plot.title = element_text(hjust = 0.5, face = "bold",colour="palegreen3"),
        ...
        axis.title.y = element_text(face = "bold",colour="palegreen3"))   # Choisir les couleurs des barres

```

```{r}
label_COHABITATION[2:5]

as.numeric(resultat[2:5])
```
