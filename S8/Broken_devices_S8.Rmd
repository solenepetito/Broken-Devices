...
# **SEMAINE DU 22 JUILLET 2024 au 26 JUILLET 2024 :**
...
-   Vert : Leproblème est résolu

-   Jaune : Le problème n'a pas pu être résolu

-   Gris : Le problème s'est résolu tout seul au bout d'un moment

-   Orange : Le problème sera peut-être résolu plus tard

-   Rouge : Je ne pense pas tenter de résoudre le problème
...
Les graphes qui suivent permettent de visualiser pour chacune des
...
### **IV ) Intention de remplacement** 

#### **1 ) Intention de remplacement en fonction de l'année d'obtention du smartphone**
...
#### **2 ) Intention de remplacement en fonction de la durée d'utilisation précédente du téléphone d'occasion**
...
### **V ) Degrés de gêne et temps de cohabitation par catégorie de dysfonctionnements**
...
### **VI ) Catégorie de dysfonctionnements**
...
val = unique(dataframe$Categorie)
print(val)
liste = c()

for (j in 1:length(val)){
    cat = val[j]
    don = dataframe[dataframe$Categorie==cat,]
    res = round(sum(don$nb))
    liste=c(liste,res)
...
df = data.frame(Catégorie = val,Nb_dys = liste, Pourcent = round(liste*100/sum(liste) ))
df
```
