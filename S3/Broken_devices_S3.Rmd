...
Diagramme en barres pour chaque CSP en fonction de l'intention de réparation : Vous nous avez déclaré ne pas avoir tenté de résoudre les dysfonctionnements suivants de votre smartphone. Envisagez-vous de tenter de le(s) résoudre ? 
...
Distribution du nombre de personnes par temps de cohabitation : Combien de temps s’est écoulé approximativement entre le moment où vous avez décelé le(s) dysfonctionnement(s) et le moment où vous avez tenté de le(s) résoudre ou de le(s) faire résoudre ?
...
```{r}
as.matrix(table(data[which(data$REPARATION_A10==1 | data$REPARATION_A10==3 ),"COHABITATION_DYSFONCTION_A10"]))
```

```{r}
boxplot(x=data[which(data$REPARATION_A10==1 | data$REPARATION_A10==3 ),"COHABITATION_DYSFONCTION_A10"])
```

```{r}
boxplot(data[,c(106:130)])
```


Nombre de personnes par modalités pour le temps de cohabitation 

```{r}

vec=c(1:26)
matrice = matrix(0,nrow=2,ncol=length(vec))
matrice[1,]=vec
df=data[which(data$RIDENTIFICATION_PROBLEMES!="27"),]
...
for(i in 1:ncol(matrice)){
  ...
  for (j in 1:length(variable)){
    ...
    if (pb %in% liste_pbs){
      ...
      nom_colonne=paste0("REPARATION_A",num)
      valeur=df[j,nom_colonne]
      if (!is.na(valeur) &&!is.null(valeur) && (valeur == 1 || valeur == 3)){
        matrice[2,i]=matrice[2,i]+1
      }
    }
  }
...
total_pbs_resolus <- sum(matrice[2,])
print(total_pbs_resolus)

```

```{r}
# Nombre total de problèmes mentionnés dans la base 

res <- Count_VCM(data$RIDENTIFICATION_PROBLEMES,26)
total_pbs=sum(res[2,])
print(total_pbs)
print(round(total_pbs_resolus*100/total_pbs))
```

C'est seulement 23% des problèmes recontrés au total dans la base qui ont été résolus.
...
df3 <- data.frame(Categorie = Categorie, valeurs = round(matrice[2, ]*100/total_pbs_resolus))
...
71 % des problèmes résolus l'ont été dés que le propriétaire du smartphone l'a décélé. 57% ont été rsolus en moins d'un mois et 27% entre 1 et 6 mois.
...
Récense du pb : A partir de quel moment approximativement avez-vous décelé les premiers dysfonctionnements sur votre smartphone ?
