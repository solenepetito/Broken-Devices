...
p <- ggplot(df_combined, aes(x = Annee, y = valeurs, fill = Legende)) +
  ...
  labs(title = "Dysfonctionnements rencontrés en fonction de l'année d'obtention",
       x = "Année d'obtention",
...
print(paste("Pourcentage de personnes dans la base qui rencontrent un pbs et qui ont leur smartphone depuis 5 ans :",pourcent2, "%"))
...
propriétaires ont acheté leur téléphone est continue, ce qui explique que
le nombre de personnes dans ce cas là est important, contrairement à la
ligne indiquant que le propriétaire l'a obtenu d'une autre manière, qui
ne contient qu'un seul point. Le tableau suivant donne plus de détails :
...
Total=matrice[2,]+matrice[3,]
Pourcentage=round(Total*100/nb_individus,1)
...
Regardons maintenant le nombre de personnes ayant rencontrés un ou
...
On observe que la part de dysfonctionnement est plus grande pour la
...
frame=data.frame(Acquisition=label_ACQUISITION, `Pourcentage de personnes avec dys` =mat[3,])
print(frame)
```
...
La question poséée était : **Avez-vous tenté de résoudre ou de faire résoudre le(s) dysfonctionnement(s) rencontré(s) ? **
...
df3 <- data.frame(Categorie = Categorie, valeurs = round(matrice_REP[2, ]*100/total_pbs))

# Créer un diagramme en barres empilées
p <- ggplot(df3, aes(x = Categorie, y = valeurs)) +
  geom_bar(stat = "identity",fill="darkgreen") + 
  labs(title = "Résolution des dysfonctionnements ",
       x = "Résolution des dysfonctionnements",
...
p_interactif 
```
 Les problèmes ayant tenté d'être résolus représente 32% des problèmes rencontrés et ceux ayant été résolus représente 18%.
...
Il serait intéressant de savoir parmis ces 10 dysfonctionnements combien ont tenté dêtre résout et ont été résolu ? Le tableau suivant nous apporte plus de détails : 
...
df <- data.frame(Dysfonctionnement=label_RIDENTIFICATION_PROBLEME[top_indices], 'Nb_personnes'=top_mat[2,],'Nb_personnes_résolu'=matrice[2,],'Pourcentage_Nb_personnes_résolu'=round(matrice[2,]*100/top_mat[2,]))
print(df)
```

La plus grande part observée concerne le dysfonctionnement "Problème de chargement" qui pour 33 pbs observés a a tenté d'être résout et a été résolu dans 10 de ces cas, ce qui représente une part de 30% de résolution. Le graphe suivant permet de visualiser cette répartiton :
...
#### **3 ) Combien de personnes qui n'ont pas réparer cohabitent avec le problème ?**
...
df <- data[which(data$SEXE==2),c(106:130)]
...
for (k in 1:ncol(matrice)){
  for (i in 1 :ncol(df)){
    for (j in 1:nrow(df)){
      val=df[j,i]
      if(!is.na(val) && val==k){
        matrice_femmes[2,k]=matrice_femmes[2,k]+1
      }
    }
  }
}
...
df <- data[which(data$SEXE==1),c(106:130)]
...
for (k in 1:ncol(matrice)){
  for (i in 1 :ncol(df)){
    for (j in 1:nrow(df)){
      val=df[j,i]
      if(!is.na(val) && val==k){
        matrice_hommes[2,k]=matrice_hommes[2,k]+1
      }
    }
  }
}
...
que les femmes semblent le régler dès le dysfonctionnement est décelé (22%).

Autre représentation mais sous forme de boites à moustache (distribution du nombre de personnes par temps de cohabitation) : 

```{r}

# Sélectionner les colonnes 106 à 130
data_long <- data %>% pivot_longer(cols = 106:130, names_to = "Variable", values_to = "Value")
don <- data.frame(Variable=data_long$Variable,COHABITATION <- factor(data_long$Value, levels = c(1:6), labels = label_COHABITATION))

# Créer le boxplot avec ggplot2
p <- ggplot(don, aes(x = Variable, y = COHABITATION)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Boxplot des colonnes 106 à 130",
       x = "Variables",
       y = "Valeurs") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        axis.title.x = element_text(face = "bold"),
...
gênant, 32% comme étant trés gênant, 18% comme étant plutôt pas gênant et
3% comme pas gênant du tout.

### **VII ) Récence du dysfonctionnement**

La question posée était :**A partir de quel moment approximativement avez-vous
décelé les premiers dysfonctionnements sur votre smartphone ? **
...
On voit que lorsque le dysfonctionnement a été décelé dés son apparition, le problème a été ressentis comme étant trés gênant. Lorsque le propriétaire a cohabité avec le dysfonctionnement moins d'un mois le dysfonctionnement comme étant majoritairement plutôt gênant.
...
Diagramme en barres pour chaque CSP en fonction de l'intention de
réparation : Vous nous avez déclaré ne pas avoir tenté de résoudre les
dysfonctionnements suivants de votre smartphone. Envisagez-vous de
tenter de le(s) résoudre ?

1 : Oui, par moi-même 2 : Oui, par le SAV ou un réparateur 3 : Oui, par
une connaissance 4 : Pas pour l'instant 5 : Non
